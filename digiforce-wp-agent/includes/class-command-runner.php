<?php
/**
 * Executes remote commands received over REST.
 *
...
class CommandRunner {
	...
	public function handle( string $command_id, string $action, array $payload ) : array {
		if ( '' === $command_id ) {
			return $this->failure( '', $action, 'missing_command_id', __( 'Missing command_id.', 'digiforce-wp-agent' ) );
		}
		if ( '' === $action ) {
			return $this->failure( $command_id, '', 'missing_action', __( 'Missing action.', 'digiforce-wp-agent' ) );
		}
		if ( ! in_array( $action, self::SUPPORTED_ACTIONS, true ) ) {
			return $this->failure( $command_id, $action, 'unsupported_action', __( 'Action is not supported.', 'digiforce-wp-agent' ) );
		}
		...
		try {
			...
			$this->logger->log( Logger::LEVEL_ERROR, Logger::CATEGORY_COMMAND, $action, 'command', $command_id, $e->getMessage() );
			return $this->failure( $command_id, $action, 'internal_server_error', $e->getMessage() );
		}
	}
	...
	private function sync_status( string $command_id ) : array {
		...
		$this->logger->log( Logger::LEVEL_INFO, Logger::CATEGORY_COMMAND, 'sync_status', 'command', $command_id, __( 'Status snapshot returned.', 'digiforce-wp-agent' ) );
		return $this->success( $command_id, 'sync_status', __( 'Status returned.', 'digiforce-wp-agent' ), $snapshot );
	}

	private function scan_updates( string $command_id ) : array {
		$summary = $this->scanner->summary( true );
		$this->logger->log( Logger::LEVEL_INFO, Logger::CATEGORY_COMMAND, 'scan_updates', 'command', $command_id, __( 'Forced update scan.', 'digiforce-wp-agent' ) );
		return $this->success( $command_id, 'scan_updates', __( 'Update scan completed.', 'digiforce-wp-agent' ), $summary );
	}

	private function update_plugin( string $command_id, array $payload ) : array {
		if ( ! $this->settings->get( 'allow_remote_plugin_updates', true ) ) {
		...
		if ( ! $result['success'] ) {
			$this->logger->log( Logger::LEVEL_ERROR, Logger::CATEGORY_UPDATE, 'update_plugin', 'plugin', $plugin_file, $result['message'] );
	...
	private function bulk_update_plugins( string $command_id, array $payload ) : array {
		if ( ! $this->settings->get( 'allow_bulk_updates', true ) ) {
	...
	private function activate_plugin_action( string $command_id, array $payload ) : array {
		if ( ! $this->settings->get( 'allow_remote_activation', true ) ) {
		...
		if ( is_wp_error( $result ) ) {
			$this->logger->log( Logger::LEVEL_ERROR, Logger::CATEGORY_COMMAND, 'activate_plugin', 'plugin', $plugin_file, $result->get_error_message() );
		...
		$this->logger->log( Logger::LEVEL_INFO, Logger::CATEGORY_COMMAND, 'activate_plugin', 'plugin', $plugin_file, __( 'Activated.', 'digiforce-wp-agent' ) );
		return $this->success( $command_id, 'activate_plugin', __( 'Plugin activated.', 'digiforce-wp-agent' ), array( 'plugin_file' => $plugin_file ) );
	}

	private function deactivate_plugin_action( string $command_id, array $payload ) : array {
		if ( ! $this->settings->get( 'allow_remote_deactivation', true ) ) {
		...
		// Never let the agent deactivate itself — that would cut off all further control.
		...
		$this->logger->log( Logger::LEVEL_INFO, Logger::CATEGORY_COMMAND, 'deactivate_plugin', 'plugin', $plugin_file, __( 'Deactivated.', 'digiforce-wp-agent' ) );
		return $this->success( $command_id, 'deactivate_plugin', __( 'Plugin deactivated.', 'digiforce-wp-agent' ), array( 'plugin_file' => $plugin_file ) );
	}
	...
	private function enable_auto_update( string $command_id, array $payload ) : array {
		...
		$this->logger->log( Logger::LEVEL_INFO, Logger::CATEGORY_COMMAND, 'enable_plugin_auto_update', 'plugin', $plugin_file, __( 'Auto-update enabled.', 'digiforce-wp-agent' ) );
		return $this->success( $command_id, 'enable_plugin_auto_update', __( 'Auto-update enabled.', 'digiforce-wp-agent' ), array( 'plugin_file' => $plugin_file ) );
	}
	...
	private function disable_auto_update( string $command_id, array $payload ) : array {
		...
		$this->logger->log( Logger::LEVEL_INFO, Logger::CATEGORY_COMMAND, 'disable_plugin_auto_update', 'plugin', $plugin_file, __( 'Auto-update disabled.', 'digiforce-wp-agent' ) );
		return $this->success( $command_id, 'disable_plugin_auto_update', __( 'Auto-update disabled.', 'digiforce-wp-agent' ), array( 'plugin_file' => $plugin_file ) );
	}
	...
	/**
	 * Execute a single-plugin upgrade using WP_Upgrader.
	 *
	...
	private function run_plugin_update( string $plugin_file ) : array {
		...
		// Refresh update data before invoking the upgrader.
		wp_update_plugins();
		...
		// Re-read plugin metadata to discover the new version.
