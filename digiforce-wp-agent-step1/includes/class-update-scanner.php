 ...
 *
 * Reusable by both the admin page and the REST controller.
 *
...
class UpdateScanner {
	...
	private const AUTO_UPDATE_PLUGINS_OPTION = 'auto_update_plugins';
	private const AUTO_UPDATE_THEMES_OPTION  = 'auto_update_themes';

	/**
	 * Force-refresh WordPress update transients.
	 */
	...
	/**
	 * Full snapshot: per-plugin, per-theme, and core arrays.
	 *
	 * @return array{plugins:array,themes:array,core:array,scanned_at:int}
	 */
	...
	/**
	 * Condensed summary suitable for /scan responses and the admin UI.
	 */
	...
	public function scan_plugins() : array {
		...
		foreach ( $all as $plugin_file => $data ) {
			...
			$out[] = array(
				...
				'auto_update_enabled' => $this->is_plugin_auto_update_enabled( $plugin_file ),
	...
	public function scan_themes() : array {
		...
		foreach ( $themes as $stylesheet => $theme ) {
			...
			$out[] = array(
				...
				'auto_update_enabled' => $this->is_theme_auto_update_enabled( (string) $stylesheet ),
			);
		}
		return $out;
	}
		...
		if ( is_array( $updates ) && ! empty( $updates ) ) {
			foreach ( $updates as $u ) {
				...
				if ( ! empty( $u->response ) && 'upgrade' === $u->response && ! empty( $u->current ) ) {
					...
					$update_type = ! empty( $u->partial_version ) ? 'partial' : 'full';
					break;
				}
			}
		}
	...
	/* ------------------------------------------------------------------ */
	/*  Auto-update helpers (inlined; a dedicated manager arrives later).  */
	/* ------------------------------------------------------------------ */

	private function is_plugin_auto_update_enabled( string $plugin_file ) : bool {
		$enabled = (array) get_site_option( self::AUTO_UPDATE_PLUGINS_OPTION, array() );
		return in_array( $plugin_file, $enabled, true );
	}

	private function is_theme_auto_update_enabled( string $stylesheet ) : bool {
		$enabled = (array) get_site_option( self::AUTO_UPDATE_THEMES_OPTION, array() );
