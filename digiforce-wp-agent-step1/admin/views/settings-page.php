...
$notice_messages = array(
	'saved'            => array( 'success', __( 'Settings saved.', 'digiforce-wp-agent' ) ),
	'uuid_regenerated' => array( 'success', __( 'A new Site UUID has been generated.', 'digiforce-wp-agent' ) ),
	'secret_rotated'   => array( 'warning', __( 'The secret key has been rotated. Update your central server immediately.', 'digiforce-wp-agent' ) ),
	'scanned'          => array( 'success', __( 'Update scan completed. Counts below are fresh.', 'digiforce-wp-agent' ) ),
	'sync_placeholder' => array( 'info', __( 'Sync request recorded. The sync engine will be implemented in a later step.', 'digiforce-wp-agent' ) ),
);

$plugins_need       = (int) $site_info['plugins_need_update'];
$themes_need        = (int) $site_info['themes_need_update'];
$core_need          = (bool) $site_info['core_update_available'];
$core_info          = isset( $site_info['core'] ) && is_array( $site_info['core'] ) ? $site_info['core'] : array();
$plugin_count_class = $plugins_need > 0 ? 'digiforce-wpa-count digiforce-wpa-count--warn' : 'digiforce-wpa-count';
$theme_count_class  = $themes_need > 0 ? 'digiforce-wpa-count digiforce-wpa-count--warn' : 'digiforce-wpa-count';
?>
<div class="wrap digiforce-wpa-wrap">
	...
	<?php if ( '' !== $notice && isset( $notice_messages[ $notice ] ) ) : ?>
		<?php list( $notice_type, $notice_text ) = $notice_messages[ $notice ]; ?>
	...
	<div class="digiforce-wpa-grid">
		...
		<div class="digiforce-wpa-card">
			...
			<table class="widefat striped digiforce-wpa-info">
				<tbody>
					...
					<tr>
						...
						<td><?php echo esc_html( Utils::format_timestamp( (int) $site_info['last_sync_at'] ) ); ?></td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'Last Scan', 'digiforce-wp-agent' ); ?></th>
						<td><?php echo esc_html( Utils::format_timestamp( (int) $site_info['last_scan_at'] ) ); ?></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><span class="<?php echo esc_attr( $plugin_count_class ); ?>"><?php echo esc_html( (string) $plugins_need ); ?></span></td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'Themes Needing Updates', 'digiforce-wp-agent' ); ?></th>
						<td><span class="<?php echo esc_attr( $theme_count_class ); ?>"><?php echo esc_html( (string) $themes_need ); ?></span></td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'Core Update Available', 'digiforce-wp-agent' ); ?></th>
						<td>
							<?php if ( $core_need ) : ?>
								<span class="digiforce-wpa-count digiforce-wpa-count--warn"><?php esc_html_e( 'Yes', 'digiforce-wp-agent' ); ?></span>
								<?php if ( ! empty( $core_info['current_version'] ) && ! empty( $core_info['latest_version'] ) ) : ?>
									<small>
										<?php
										echo esc_html(
											sprintf(
												/* translators: 1: current WP version, 2: latest WP version */
												__( '%1$s → %2$s', 'digiforce-wp-agent' ),
												(string) $core_info['current_version'],
												(string) $core_info['latest_version']
											)
										);
										?>
									</small>
								<?php endif; ?>
							<?php else : ?>
								<?php esc_html_e( 'No', 'digiforce-wp-agent' ); ?>
							<?php endif; ?>
						</td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'Signed Requests Required', 'digiforce-wp-agent' ); ?></th>
						<td>
							<?php if ( ! empty( $site_info['signed_requests_required'] ) ) : ?>
								<span class="digiforce-wpa-badge on"><?php esc_html_e( 'Yes', 'digiforce-wp-agent' ); ?></span>
							<?php else : ?>
								<span class="digiforce-wpa-badge off"><?php esc_html_e( 'No (dev mode)', 'digiforce-wp-agent' ); ?></span>
							<?php endif; ?>
						</td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'Remote Plugin Updates', 'digiforce-wp-agent' ); ?></th>
						<td>
							<?php if ( ! empty( $site_info['remote_updates_allowed'] ) ) : ?>
								<span class="digiforce-wpa-badge on"><?php esc_html_e( 'Allowed', 'digiforce-wp-agent' ); ?></span>
							<?php else : ?>
								<span class="digiforce-wpa-badge off"><?php esc_html_e( 'Denied', 'digiforce-wp-agent' ); ?></span>
							<?php endif; ?>
							<?php if ( ! empty( $site_info['bulk_updates_allowed'] ) ) : ?>
								<small>· <?php esc_html_e( 'bulk updates on', 'digiforce-wp-agent' ); ?></small>
							<?php else : ?>
								<small>· <?php esc_html_e( 'bulk updates off', 'digiforce-wp-agent' ); ?></small>
							<?php endif; ?>
						</td>
					</tr>
				</tbody>
			...
			<div class="digiforce-wpa-actions">
				<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="digiforce-wpa-inline-form" data-scan-form="1">
				...
				<table class="form-table">
					...
					<tr>
						...
						<td>
							...
							<p class="description"><?php esc_html_e( 'Comma-separated list of IPs allowed to reach the REST endpoints. Leave empty to allow all.', 'digiforce-wp-agent' ); ?></p>
						</td>
					</tr>
					<tr>
