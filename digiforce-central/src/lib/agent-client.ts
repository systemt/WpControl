...
import { signAgentRequest } from './security/hmac';
...
export const AGENT_COMMAND_ROUTE = '/digiforce-agent/v1/command';
const AGENT_COMMAND_REST_PATH = '/wp-json/digiforce-agent/v1/command';

export interface SendAgentCommandParams {
  site: { url: string; uuid: string };
  secret: string;
  commandId: string;
  action: string;
 ...
 */
export async function sendAgentCommand(params: SendAgentCommandParams): Promise<AgentCommandResult> {
